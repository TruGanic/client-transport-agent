...
export default function HomeStatusCarousel({
  ...
  const CARDS = [{ type: "OVERVIEW" }, { type: "TEMP" }, { type: "HUMIDITY" }];
...
const StatusCard = ({
        ...
        <View className="flex-row items-center justify-between border-t border-white/10 pt-3 mt-2">
          <View>
            ...
            <Text className="text-white text-base font-bold">
              {isRecording ? bufferSize : "--"}
            </Text>
          </View>
          <View>
            <Text className="text-white text-[10px] uppercase font-bold text-right">
              Last Sync
            </Text>
            <Text className="text-white text-base font-bold text-right">
              {lastSyncTime}
            </Text>
          </View>
        </View>
      ...
      <View className="flex-1 justify-between">
        ...
        {isRecording && (
          <View className="self-start">
            <View
              className={`px-3 py-1.5 rounded-full flex-row items-center ${currentTemp && currentTemp > 2 && currentTemp < 12 ? "bg-teal-800/40" : "bg-yellow-500/20"}`}
            >
              <Ionicons
                name={
                  currentTemp && currentTemp > 2 && currentTemp < 12
                    ? "checkmark-circle"
                    : "alert-circle"
                }
                size={14}
              ...
              />
              <Text
                className={`text-xs font-bold ml-1 ${currentTemp && currentTemp > 2 && currentTemp < 12 ? "text-green-300" : "text-orange-200"}`}
              >
                {currentTemp && currentTemp > 2 && currentTemp < 12
                  ? "Optimal Range"
                  : "Warning"}
        ...
        {!isRecording && (
          <Text className="text-white text-sm">Sensor inactive</Text>
        )}
      </View>
    );
  }
  ...
  if (item.type === "HUMIDITY") {
    ...
    Content = (
      <View className="flex-1 justify-between">
        ...
        <View className="flex-row items-baseline">
          <Text className="text-white text-3xl font-bold">
            {isRecording && currentHumidity !== null
        ...
        {isRecording && (
          <View className="self-start">
            <View className="px-3 py-1.5 rounded-full flex-row items-center bg-blue-900/30">
              ...
              <Text className="text-xs font-bold ml-1 text-white">
                Relative Level
              </Text>
            </View>
          </View>
  ...
  return (
    <View style={{ width: width, alignItems: "center" }}>
      <Animated.View
        className="w-full h-full p-6 shadow-xl shadow-green-900/30 relative"
        style={[
          {
            width: CARD_WIDTH,
            height: CARD_HEIGHT,
