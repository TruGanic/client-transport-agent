  ...
  return (
    <ScrollView className="flex-1 bg-background">
      ...
      <View className="px-6 pt-8 pb-4 bg-surface rounded-b-3xl shadow-sm border-b border-gray-100">
        ...
        {/* Status Pill */}
        <View
          className={`self-start px-3 py-1 rounded-full flex-row items-center ${statusBg}`}
        >
          <View
            className={`w-2 h-2 rounded-full mr-2 ${isRecording ? "bg-green-600 animate-pulse" : "bg-gray-400 animate-none"}`}
          />
      ...
      <View className="mt-6">
        ...
        <View className="px-5">
          ...
          <Text className="text-textPrimary text-lg font-bold mt-6 mb-3">
            Recent Logs
          </Text>
          <View className="bg-surface rounded-xl p-4 shadow-sm border border-gray-100 mb-10">
            {logs.length === 0 ? (
              ...
              logs.slice(0, 3).map((log, i) => (
                <View
                  key={i}
                  className="flex-row items-center border-b border-gray-50 py-2 last:border-0"
                >
